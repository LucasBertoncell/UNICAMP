...
int imprimir_personagens(char personagens[], int numero){
	...
	for (i = PosLagartaAnterior + 1; i <= numero; i++){
		if (ignorado[i] != 'i'){
...
int main (){
	...
	unsigned int i, entrada, tempo = 0, jogou_criquete = 0, ganhou_criquete = 0;
	unsigned int segundos = 0, min = 0, cooldown = 0;
	int MORREU = 0, SAIU = 0, numero = 1, PosLagarta = 0;

	char tipo, personagens[100], ignorado[100];
	/*
	...
	for (i = 0; i == 0;){
		...
		*/
		...
		if(qual_personagem(min) != 'X'){
			...
			if (personagens[numero] == personagens[numero-1] && cooldown < 60){
...
/*-----------------------PERSONAGEMS EFECT -----------------------------------------------*/	
			...
			if (qual_personagem(min) == 'L'){
				...
				PosLagartaAnterior = imprimir_personagens(personagens, numero, PosLagartaAnterior, ignorado);
